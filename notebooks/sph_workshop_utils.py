...
def check_swift_installation(swift_executable='swift'):
    """
    Check if SWIFT is installed and available
    
    Parameters:
    -----------
    swift_executable : str
        Name or path to SWIFT executable
        
    Returns:
    --------
    bool : True if SWIFT is available, False otherwise
    """
    try:
        result = subprocess.run(['which', swift_executable], 
                               capture_output=True, text=True)
        if result.returncode == 0:
            print(f"SWIFT found at: {result.stdout.strip()}")
            return True
        else:
            print(f"SWIFT executable '{swift_executable}' not found.")
            return False
    except Exception as e:
        print(f"Error checking SWIFT installation: {e}")
        return False
    ...
    try:
        with h5py.File(filename, 'r') as f:
            ...
            data = {
                ...
                'smoothing_length': part0['SmoothingLength'][:] if 'SmoothingLength' in part0 else None,
                'masses': part0['Masses'][:]
            }
...
def calculate_gravitational_softening(particle_positions, softening_fraction=0.05):
    ...
    if len(particle_positions.shape) == 2:
        ...
        if particle_positions.shape[1] == 2:
            ...
            # 3D case  
            volume = (np.ptp(particle_positions[:, 0]) * 
